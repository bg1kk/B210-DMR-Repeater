...
源码与Pi5部署版本：`V1.2.1 B167`。该版本将活动信道和快捷信道列表持久化到配置文件，快捷信道按信道号升序显示，GUI主页的频率数字使用DashLCDSegment数码字体并与RX/TX标签、MHz单位分栏显示；RX/TX频率占主页信道卡片主要区域；切换信道期间允许继续操作其他页面控件，并过滤旧切换响应，保证多次切换后活动信道、反色状态和频率在所有页面同步；转发运行状态前的圆点在运行时显示红色、待机时显示背景色；IO4/IO5继续明确为读取外部前级档位的输入，绝对dBm仍须完成现场全点校准后才有效。
