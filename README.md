...
源码与Pi5部署版本：`V1.2.1 B174`。该版本在GUI启动确认 `forwarding_enabled=true`、`rf_running=true` 和活动信道后显示“转发已激活，转发频道为CHx”，并兼容B210射频会话晚于GUI状态订阅完成的启动时序；所有“等待确认”操作均以成功、失败、超时或被后续请求替代事件闭环，启动/停止转发和信道切换以状态流作为最终成功依据。主页继续使用DashLCDSegment数码字体和统一的RX/TX标签、数字栏及MHz间距。
